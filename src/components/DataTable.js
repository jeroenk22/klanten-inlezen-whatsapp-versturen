...
export default function DataTable({ data, titleDate, onInputChange, onCopy, onReset }) {
  ...
  const renderCell = (cell, rowIndex, cellIndex) => {
    if (cellIndex === 11) { // Alleen voor de Mobiel-kolom (index 11)
      return (
        <input
          type="text"
  ...
  return (
    <div className="mt-4 border p-2">
      ...
      <div className="mb-2">
        <Button onClick={() => onCopy(1)} text="Kopieer Ordernrs" color="bg-blue-500" />
        <Button onClick={() => onCopy(2)} text="Kopieer Sjabloonnrs" color="bg-green-500" />
        <Button onClick={() => onCopy(3)} text="Kopieer Taaknrs" color="bg-yellow-500" />
        <Button onClick={() => copyInvalidNumbersToClipboard(data, 1, "Ordernummers met foutieve mobiele nummers gekopieerd naar klembord!")} 
          text="Kopieer Ordernrs met foutief mobiel nummer" color="bg-red-500" />
        <Button onClick={() => copyInvalidNumbersToClipboard(data, 2, "Sjabloonnrs met foutieve mobiele nummers gekopieerd naar klembord!")} 
          text="Kopieer Sjabloonnrs met foutief mobiel nummer" color="bg-red-600" />
        <Button onClick={onReset} text="Reset" color="bg-gray-500" />
        <Button onClick={() => setIsModalOpen(true)} text="Open WhatsApp Modal" color="bg-green-600" />
      </div>
